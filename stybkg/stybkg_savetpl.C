...
void stybkg_savetpl(TString infname, TString outfname, 
                     ...
                     Float_t leading_ptmin, Float_t other_ptmin, Float_t leading_trkptmin)
{
  ...
  for(int i=0;i<nentries;i++)
    {
      if(i%10000==0) std::cout<<std::setiosflags(std::ios::left)<<"  [ \033[1;36m"<<std::setw(10)<<i<<"\033[0m"<<" / "<<std::setw(10)<<nentries<<" ] "<<"\033[1;36m"<<std::setw(4)<<Form("%.0f%s",100.*i/nentries,"%")<<"\033[0m"<<"   >>   stybkg_savetpl("<<std::setw(20)<<Form("%s)",collisionsyst.Data())<<"\r"<<std::flush;
...
int main(int argc, char* argv[])
{
  if(argc==7)
    {
      stybkg_savetpl(argv[1], argv[2], argv[3], atof(argv[4]), atof(argv[5]), atof(argv[6]));
      return 0;
    }
  else
    {
      std::cout<<"  Error: invalid arguments number - stybkg_savetpl()"<<std::endl;
      return 1;
    }
}
