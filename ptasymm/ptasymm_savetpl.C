...
void ptasymm_savetpl(TString infname, TString outfname, 
                     ...
                     Float_t leading_ptmin, Float_t other_ptmin, Float_t leading_trkptmin)
{
  ...
  for(int i=0;i<nentries;i++)
    {
      if(i%10000==0) std::cout<<std::setiosflags(std::ios::left)<<"  [ \033[1;36m"<<std::setw(10)<<i<<"\033[0m"<<" / "<<std::setw(10)<<nentries<<" ] "<<"\033[1;36m"<<std::setw(4)<<Form("%.0f%s",100.*i/nentries,"%")<<"\033[0m"<<"   >>   ptasymm_savetpl("<<std::setw(5)<<Form("%s,",collisionsyst.Data())<<" "<<std::setw(20)<<Form("%s)",tMC[isMC].Data())<<"\r"<<std::flush;
      ...
      for(std::map<int, double>::iterator it=dphi.begin(); it!=dphi.end(); it++)
        {
          ...
          if(iptasym<0) {std::cout<<"error: check ptasymm value: "<<dcand.Dpt[it->first]/dcand.Dpt[jleading]<<std::endl; return;}
          if(dcand.Dgen[it->first]==23333) ahmassSignal[iptasym]->Fill(dcand.Dmass[it->first]);
          if(dcand.Dgen[it->first]==23344) ahmassSwapped[iptasym]->Fill(dcand.Dmass[it->first]);
        }
    }
  std::cout<<std::setiosflags(std::ios::left)<<"  Processed "<<"\033[1;31m"<<nentries<<"\033[0m event(s)."<<"   >>   ptasymm_savetpl("<<std::setw(5)<<Form("%s,",collisionsyst.Data())<<" "<<std::setw(30)<<Form("%s)",tMC[isMC].Data())<<std::endl;
  std::cout<<std::endl;
...
int main(int argc, char* argv[])
{
  if(argc==8)
    {
      ptasymm_savetpl(argv[1], argv[2], argv[3], atoi(argv[4]), atof(argv[5]), atof(argv[6]), atof(argv[7]));
      return 0;
    }
  else
    {
      std::cout<<"  Error: invalid arguments number - ptasymm_savetpl()"<<std::endl;
      return 1;
    }
}
